...
# 02_regist_mcmp_apis.sh - Register MCMP APIs
# MCMP API 등록 스크립트
...
show_usage() {
  ...
  echo ""
  echo "This script registers MCMP APIs via API call."
...
# API endpoint
API_URL="${MCIAMMANAGER_HOST}/api/setup/sync-mcmp-apis"

echo "Registering MCMP APIs..."
...
if [[ $http_code -eq 200 ]] || [[ $http_code -eq 201 ]]; then
  echo ""
  echo "✅ MCMP APIs registration completed successfully!"
  exit 0
else
  echo ""
  echo "❌ Failed to register MCMP APIs. HTTP Status: $http_code"
  exit 1
fi 