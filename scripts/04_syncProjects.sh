...
# 04_syncProjects.sh - Sync projects
# 프로젝트 동기화 스크립트
...
show_usage() {
  ...
  echo ""
  echo "This script syncs projects via API call."
...
# API endpoint
API_URL="${MCIAMMANAGER_HOST}/api/setup/sync-projects"
...
if [[ $http_code -eq 200 ]] || [[ $http_code -eq 201 ]]; then
  echo ""
  echo "✅ Projects sync completed successfully!"
  exit 0
else
  echo ""
  echo "❌ Failed to sync projects. HTTP Status: $http_code"
  exit 1
fi 